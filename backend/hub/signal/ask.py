...
def digest(region: str, platform: str | None, market: str | None) -> dict:
    """
    ...
    hiện một nẻo.
    """
    sig = trendsig.build(region, store.get_config("trends"), up_only=True)
    lines: list[str] = []
    index: dict[str, dict] = {}
    ...
    for row in sig["rows"][:MAX_SIGNAL_HINTS]:
        ...
        index[_fold(row["keyword"])] = {
            "source": "trends", "kind": row["kind"], "heat": row["heat"],
...
async def ask(turns: list[dict], region: str = "VN",
    ...
    """
    chat: list[ChatTurn] = []
    dg = digest(region, platform, market)
    if dg["text"]:
