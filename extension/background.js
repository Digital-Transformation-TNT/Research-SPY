...
// Taobao KÝ SINH: để CHÍNH TRANG s.taobao.com gọi mtop bằng token của nó, ta chỉ chộp response.
//
// VÌ SAO BỎ CÁCH TỰ KÝ. Bản cũ tự dựng chữ ký md5 rồi `fetch` mtop h5search từ tab
// h5api.m.taobao.com. Cách đó chết hẳn trên máy-thợ đặt ở VPS: đo 16/09/2026 gọi thẳng từ IP
// máy chủ này, lượt ĐẦU TIÊN đã nhận `RGV587_ERROR::SM::哎哟喂,被挤爆啦` — Baxia chặn trước cả
// khi kịp phát token, nên vòng thử lại ba lần chỉ tổ mất thời gian. Đó là chống-bot của Alibaba
// nhận ra một lượt gọi API trần trụi không đi kèm phiên trình duyệt nào.
//
// Trang HTML thì KHÔNG bị chặn (đo cùng lúc: `s.taobao.com/search` trả HTTP 200, dựng đủ vỏ
// trang). Nên để trang tự chạy, tự ký, tự gửi — ta chỉ bọc `fetch`/`XMLHttpRequest` để đọc ké
// kết quả. Đúng cách `searchTemu` đang dùng, và bỏ luôn gánh nặng duy trì chữ ký md5.
//
// ĐIỀU KIỆN BẮT BUỘC, không có đường vòng: Chrome máy-thợ PHẢI đăng nhập taobao.com. Đo
// 16/09/2026 trên phiên chưa đăng nhập, trang dựng xong nhưng đứng ở "加载中..." với 0 sản phẩm
// và hiện "亲，请登录" — Taobao không cho xem kết quả tìm kiếm khi chưa đăng nhập. Không bộ đọc
// nào cứu được điều đó, nên hàm này nói thẳng ra thay vì trả bảng rỗng.

//: Bao lâu thì thôi chờ trang tự gọi mtop. Rộng hơn Temu (18s) vì s.taobao.com dựng SPA nặng
//: hơn, và máy-thợ còn chạy chung hàng đợi tab với các sàn khác.
const TAOBAO_CHO_RESPONSE_MS = 22000;
...
async function searchTaobao(keyword, count) {
  try {
    ...
    await chrome.tabs.update(tab.id, { url });
    await waitForComplete(tab.id, 15000);
    await sleep(1200); // để SPA mount xong ô search

    // Cài hook TRƯỚC, rồi mới kích hoạt một lượt tìm mới: lượt mtop lúc trang vừa tải có thể đã
    // bay qua trước khi ta kịp bọc, nên không được phụ thuộc vào nó.
    await withTimeout(chrome.scripting.executeScript({
      target: { tabId: tab.id }, world: 'MAIN', args: [keyword],
      func: (kw) => {
        if (!window.__rsTbCap) {
          window.__rsTbCap = [];
          window.__rsTbStatus = 0;
        ...
        // Gõ lại từ khoá + Enter để trang tự bắn một lượt mtop MỚI, sau khi hook đã nằm sẵn.
        try {
          const inp = document.querySelector('#q') || document.querySelector('input[name="q"]')
            || [...document.querySelectorAll('input')].find((e) => /搜索|search/i.test((e.placeholder || '') + (e.getAttribute('aria-label') || '')));
          if (inp) {
    ...
    const deadline = Date.now() + TAOBAO_CHO_RESPONSE_MS;
    let texts = [], domItems = [], status = 0, loiMtop = '', canLogin = false;
    while (Date.now() < deadline) {
      await sleep(900);
      const out = await withTimeout(chrome.scripting.executeScript({
        target: { tabId: tab.id }, world: 'MAIN',
        func: () => {
          // ĐỌC DOM LÀM ĐƯỜNG THỨ HAI. Trang có thể render sản phẩm từ dữ liệu nhúng sẵn mà
          // không phát thêm lượt mtop nào — lúc đó hook không chộp được gì nhưng màn hình vẫn
          // đầy hàng, và trả bảng rỗng khi đang nhìn thấy sản phẩm là kiểu sai khó tin nhất.
          const ra = [];
          for (const a of document.querySelectorAll('a[href*="item.taobao.com"], a[href*="detail.tmall.com"]')) {
            const m = (a.getAttribute('href') || '').match(/[?&]id=(\d+)/);
            if (!m) continue;
            const the = a.closest('[class*="item"], [class*="Card"], div');
            const chu = (the && the.innerText) || a.innerText || '';
            const img = (the && the.querySelector('img')) || a.querySelector('img');
            const gia = chu.match(/(\d[\d.,]*)\s*(?:元|¥)|¥\s*(\d[\d.,]*)/);
            const ban = chu.match(/([\d.]+万?)\+?\s*人(?:付款|购买)|已售\s*([\d.]+万?)/);
            ra.push({
              id: m[1],
              title: (img && (img.alt || '')) || chu.split('\n').filter((x) => x.trim().length > 6)[0] || '',
              img: (img && (img.src || img.getAttribute('data-src'))) || '',
              giaChu: gia ? (gia[1] || gia[2] || '') : '',
              banChu: ban ? (ban[1] || ban[2] || '') : '',
            });
          }
          return {
    ...
    await focusTab(tab.id);
    if (canLogin || /login\.taobao\.com/.test(loiMtop)) {
      return { items: [], blocked: true,
               error: 'Taobao đòi đăng nhập — đăng nhập taobao.com trong Chrome chạy máy-thợ rồi bấm Research lại' };
    }
    if (/RGV587/.test(loiMtop)) {
      return { items: [], blocked: true,
               error: 'Taobao chặn bằng Baxia (RGV587) — mở tab Taobao vừa bật, giải slider một lần rồi bấm Research lại' };
    }
    return { items: [], blocked: true,
...
//: hình ("¥89.00", "2.3万人付款") nên phải quy đổi: 万 = 10.000.
function parseTaobaoDom(raw, count) {
  const out = [];
  for (const it of raw) {
    if (!it || !it.id) continue;
    const price = parseFloat(String(it.giaChu || '').replace(/[^0-9.]/g, '')) || null;
    let monthly = parseFloat(String(it.banChu || '').replace(/[^0-9.]/g, '')) || null;
    if (monthly && /万/.test(String(it.banChu))) monthly = Math.round(monthly * 10000);
    let image = it.img || '';
    if (image.indexOf('//') === 0) image = 'https:' + image;
    out.push({ id: String(it.id), name: String(it.title || '').trim(), image,
               price, monthly, shop: '', videoUrl: '' });
...
function parseTaobaoTexts(texts, count) {
  const out = [];
  // NHẬN DIỆN RỘNG, vì nay nguồn JSON là response CỦA CHÍNH TRANG chứ không phải lượt gọi ta tự
  // dựng, và trang đời mới đặt tên field khác: `itemId` + `priceShow`/`priceWap` thay cho
  // `nid` + `view_price`. Giữ nguyên bộ tên cũ bên cạnh — mtop vẫn trả kiểu cũ ở vài lối vào.
  const coTen = (o) => o.title || o.raw_title || o.subject || o.itemTitle;
  const coGia = (o) => o.price || o.view_price || o.priceInfo || o.sortPrice || o.priceShow || o.priceWap || o.realPrice;
  const looks = (o) => o && typeof o === 'object' && (o.item_id || o.nid || o.itemId || o.id) && coTen(o) && coGia(o);
  for (const text of texts) {
    ...
    for (const it of arr) {
      ...
      if (!id) continue;
      const name = String(it.title || it.raw_title || it.subject || it.itemTitle || '').replace(/<[^>]+>/g, '').trim();
      const priceRaw = it.price || it.view_price || (it.priceInfo && (it.priceInfo.price || it.priceInfo.priceStr))
        || it.sortPrice || it.priceShow || it.priceWap || it.realPrice || '';
      const price = parseFloat(String(priceRaw).replace(/[^0-9.]/g, '')) || null;
      const soldRaw = String(it.realSales || it.view_sales || it.sold || it.payNum || it.soldNum
        || it.realSalesCount || (it.priceInfo && it.priceInfo.saleNum) || '');
