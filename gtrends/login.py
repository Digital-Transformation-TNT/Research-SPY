...
Đăng nhập Google một lần, lưu lại phiên cho server dùng.
    ...
    cd backend
    python -m gtrends.login
    python -m gtrends.login --fresh              # xoá hồ sơ Chrome, đăng nhập từ đầu
    python -m gtrends.login --name 2 --no-verify # thêm một tài khoản vào hồ
...
xem `pick_session` ở `gtrends/_auth.py`. Thêm tài khoản là việc MỘT LẦN và không phải restart
...
from ._auth import AUTH_DIR, GOOGLE_SESSION, storage_state_path
from ._browser import adopt_playwright
from ._config import config
from .context import TrendsContext
from .core import TRENDS_HOST, fetch_related_queries
...
def _session_name(slug: str) -> str:
    """`google`, `google-2`, `google-cty`… Khớp với `session_paths` ở `gtrends/_auth.py`."""
...
async def _verify(page: Page) -> tuple[str, str]:
    ...
    """
    outcome = await fetch_related_queries(PROBE_SEED, TrendsContext(country="VN"))
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            print(
                ...
                "  python -m gtrends.login --name 2 --fresh --no-verify"
            )
            return 0
        finally:
            try:
